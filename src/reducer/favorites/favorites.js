import { extend, getFavoriteHotelsData } from "../../utils";
...
const Operation = {
  getFavoriteResponse: (id, value) => (dispatch, getState, api) => {
    const valueForFavoritesServer = value
      ? FAVORITE_REQUESTS.ADD
      : FAVORITE_REQUESTS.DELETE;
    return api
      .post(`/favorite/${id}/${valueForFavoritesServer}`)
      .then(() => {
        dispatch(ActionCreator.getFavoriteResponse(false));
      })
      .catch((err) => {
        dispatch(ActionCreator.getFavoriteResponse(true));
        throw err;
      });
  },
  getFavoritesData: () => (dispatch, getState, api) => {
    return api.get(`/favorite`).then((response) => {
      dispatch(ActionCreator.getFavoritesData(response.data));
    });
  },
};
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ActionType.GET_FAVORITES_DATA:
      return extend(state, {
        favorites: getFavoriteHotelsData(action.payload),
      });
  }
  return state;
};
