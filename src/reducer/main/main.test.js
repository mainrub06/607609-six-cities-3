...
const mockActionChangeActiveFilter = ActionCreator.setActiveFilter({
...
const mockActionSetActiveOfferId = ActionCreator.setActiveOfferId({
  activeOfferId: 6,
});
...
describe(`inner app tests`, () => {
  ...
  it(`Reducer return new state(city)`, () => {
    expect(reducer(initialState, mockActionChangeCity)).toEqual(
      newStateChangeCity
    );
  });

  it(`Reducer return new state(active-filter)`, () => {
    expect(reducer(initialState, mockActionChangeActiveFilter)).toEqual(
      newStateChangeActiveFilter
    );
  });

  it(`Reducer return new state(active-hotel-id)`, () => {
    expect(reducer(initialState, mockActionSetActiveOfferId)).toEqual(
      newStateSetActiveOfferId
    );
  });
});
